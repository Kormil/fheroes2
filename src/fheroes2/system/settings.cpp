...
std::string Settings::String() const
{
    ...
    os << "landscape_upside = " << ( _gameOptions.Modes( GLOBAL_LANDSCAPE_UPSIDE ) ? "on" : "off" ) << std::endl;
...
{
    if ( enable ) {
        _gameOptions.SetModes( GLOBAL_LANDSCAPE_UPSIDE );
    } else {
        _gameOptions.ResetModes( GLOBAL_LANDSCAPE_UPSIDE );
    }
...
{
    return _gameOptions.Modes( GLOBAL_LANDSCAPE_UPSIDE );
}
